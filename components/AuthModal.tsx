...
export default function AuthModal({ isOpen, onClose, initialMode }: AuthModalProps) {
  ...
  useEffect(() => {
    // Reset emailSent whenever modal opens or closes
    setEmailSent(false);
    if (isOpen) {
      setMode(initialMode);
      setError("");
        ...
        {emailSent ? (
          <div style={{ textAlign: "center", padding: "32px" }}>
            ...
            <p style={{ color: "#666", marginBottom: "8px" }}>
              Confirmation email sent! Please check your inbox and click the confirmation link on this same device for the smoothest experience.
            </p>
            <p style={{ color: "#666", marginBottom: "16px" }}>
              If you open the link on a different device, simply return here and log in manually with your email and password.
            </p>
            ...
            <button
              type="button"
              onClick={() => { setEmailSent(false); onClose(); }}
              style={{ padding: "12px 24px", background: "#F97316", color: "white", borderRadius: "8px", border: "none", fontWeight: 600, cursor: "pointer" }}
            >
              Got it
            </button>
          </div>
