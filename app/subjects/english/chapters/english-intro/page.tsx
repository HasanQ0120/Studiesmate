...
const VIDEO_PATHS = {
  en: "simple_sentences_english.mp4.mp4",
  ur: "simple_sentence_urdu.mp4.mp4",
};
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "quizComplete") {
        ...
        console.log("[StudiesMate][ENGLISH LESSON PAGE] quizComplete received — quizId:", e.data?.quizId, "score:", score);
        console.log("[StudiesMate][ENGLISH LESSON PAGE] studiesmate_quiz_completions BEFORE:", localStorage.getItem("studiesmate_quiz_completions"));
        console.log("[StudiesMate][ENGLISH LESSON PAGE] ⚠️ This handler does NOT write to quizCompletions — unlock will NOT happen from this path");
        if (score >= 60) {
        ...
        console.log("[StudiesMate][ENGLISH LESSON PAGE] studiesmate_quiz_completions AFTER:", localStorage.getItem("studiesmate_quiz_completions"));
        console.log("[StudiesMate][ENGLISH LESSON PAGE] Expected: {\"english-intro\":true} — Actual has it?", JSON.parse(localStorage.getItem("studiesmate_quiz_completions") || "{}")["english-intro"] === true);
      }
    }
